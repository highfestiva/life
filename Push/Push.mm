...
+(void) updateContent
{
	...
	CURE_RTVAR_SET(Push::App::GetApp()->mVariableScope, RTVAR_CONTENT_LEVELS, (hasLevels == 1));

	NSInteger hasVehicles = [defaults integerForKey:@ CONTENT_VEHICLES];
	CURE_RTVAR_SET(Push::App::GetApp()->mVariableScope, RTVAR_CONTENT_VEHICLES, (hasVehicles == 1));

	NSString* objcHiscoreName = [defaults stringForKey:HISCORE_NAME_KEY];
	const str hiscoreName = MacLog::Decode(objcHiscoreName);
	CURE_RTVAR_SET(Push::App::GetApp()->mVariableScope, RTVAR_HISCORE_NAME, hiscoreName);
}

+(void) storeHiscoreName
{
	str lLastHiscoreName;
	CURE_RTVAR_GET(lLastHiscoreName, =, Push::App::GetApp()->mVariableScope, RTVAR_HISCORE_NAME, _T(""));
...
-(void) tick
{
	...
	else
	{
		...
		Push::App::GetApp()->Poll();
		[self dropFingerMovements];
	}
}

-(Push::FingerMovement&) getFingerMovement:(const CGPoint&)pLocation previous:(const CGPoint&)pPrevious
{
	Push::FingerMoveList::iterator i = Push::gFingerMoveList.begin();
	for (; i != Push::gFingerMoveList.end(); ++i)
	{
	...
	Push::gFingerMoveList.push_back(Push::FingerMovement(pLocation.x, pLocation.y));
	return Push::gFingerMoveList.back();
}

-(void) dropFingerMovements
{
	Push::FingerMoveList::iterator i = Push::gFingerMoveList.begin();
	for (; i != Push::gFingerMoveList.end();)
	{
		if (!i->mIsPress)
		{
			Push::gFingerMoveList.erase(i++);
			//return;
		}
		else
		{
...
-(void) touchesMoved:(NSSet*)touches withEvent:(UIEvent*)event
{
	...
	while ((lTouch = (UITouch*)[e nextObject]))
	{
		...
		Push::FingerMovement& lMove = [self getFingerMovement:lTapPosition previous:lPrevTapPosition];
		lMove.mIsPress = lIsPressed;
		/*Push::App::OnTap(lMove);
		if (!lIsPressed)
		{
			[self dropFingerMovement:lTapPosition previous:lPrevTapPosition];
		}*/

		//Push::App::OnMouseTap(lTapPosition.x, lTapPosition.y, lIsPressed);
	}
}
...
-(void) startPurchase:(NSString*)productName
{
	...
	{
		Push::App::GetApp()->SetIsPurchasing(true);
...
{
	self.requestedProduct = nil;
	Push::App::GetApp()->SetIsPurchasing(false);
}
