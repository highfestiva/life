...
+(void) updateContent
{
	...
	//v_set(Push::Push::GetApp()->mVariableScope, RTVAR_CONTENT_LEVELS, (hasLevels == 1));

	//NSInteger hasVehicles = [defaults integerForKey:@ CONTENT_VEHICLES];
	//v_set(Push::Push::GetApp()->mVariableScope, RTVAR_CONTENT_VEHICLES, (hasVehicles == 1));

	//NSString* objcHiscoreName = [defaults stringForKey:HISCORE_NAME_KEY];
	//const str hiscoreName = MacLog::Decode(objcHiscoreName);
	//v_set(Push::Push::GetApp()->mVariableScope, RTVAR_HISCORE_NAME, hiscoreName);
}

+(void) storeHiscoreName
{
	//str lLastHiscoreName;
	//v_get(lLastHiscoreName, =, Push::Push::GetApp()->mVariableScope, RTVAR_HISCORE_NAME, _T(""));
