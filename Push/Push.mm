...
+(void) updateContent
{
	...
	//CURE_RTVAR_SET(Push::Push::GetApp()->mVariableScope, RTVAR_CONTENT_LEVELS, (hasLevels == 1));

	//NSInteger hasVehicles = [defaults integerForKey:@ CONTENT_VEHICLES];
	//CURE_RTVAR_SET(Push::Push::GetApp()->mVariableScope, RTVAR_CONTENT_VEHICLES, (hasVehicles == 1));

	//NSString* objcHiscoreName = [defaults stringForKey:HISCORE_NAME_KEY];
	//const str hiscoreName = MacLog::Decode(objcHiscoreName);
	//CURE_RTVAR_SET(Push::Push::GetApp()->mVariableScope, RTVAR_HISCORE_NAME, hiscoreName);
}

+(void) storeHiscoreName
{
	//str lLastHiscoreName;
	//CURE_RTVAR_GET(lLastHiscoreName, =, Push::Push::GetApp()->mVariableScope, RTVAR_HISCORE_NAME, _T(""));
