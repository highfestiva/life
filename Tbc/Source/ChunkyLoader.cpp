...
bool ChunkyLoader::AllocLoadChunkyList(FileElementList& pLoadList, int64 pChunkEnd)
{
	...
	while (!lLoadedAllElements && lOk && mFile->Tell() < pChunkEnd)
	{
		...
		if (lOk)
		{
			...
			deb_assert(lOk);
#define C(t)	lType == t
			deb_assert(C(CHUNK_MESH) || C(CHUNK_MESH_VERTICES) || C(CHUNK_MESH_NORMALS) || C(CHUNK_MESH_UV) || C(CHUNK_MESH_UVS_PER_VERTEX) ||
				C(CHUNK_MESH_COLOR) || C(CHUNK_MESH_COLOR_FORMAT) || C(CHUNK_MESH_TRIANGLES) ||
				C(CHUNK_MESH_PRIMITIVE) || C(CHUNK_MESH_VOLATILITY) || C(CHUNK_MESH_CASTS_SHADOWS) || C(CHUNK_MESH_SHADOW_DEVIATION) || C(CHUNK_MESH_TWO_SIDED) || C(CHUNK_MESH_RECV_NO_SHADOWS) ||
