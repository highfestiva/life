...
+(void) updateContent {
	...
	//v_set(Downwash::Downwash::GetApp()->variable_scope_, kRtvarContentLevels, (hasLevels == 1));

	//NSInteger hasVehicles = [defaults integerForKey:@ kContentVehicles];
	//v_set(Downwash::Downwash::GetApp()->variable_scope_, kRtvarContentVehicles, (hasVehicles == 1));

	//NSString* objcHiscoreName = [defaults stringForKey:HISCORE_NAME_KEY];
	//const str hiscoreName = MacLog::Decode(objcHiscoreName);
	//v_set(Downwash::Downwash::GetApp()->variable_scope_, kRtvarHiscoreName, hiscoreName);
}

+(void) hiscore_name_
{
	//str last_hiscore_name;
	//v_get(last_hiscore_name, =, Downwash::Downwash::GetApp()->variable_scope_, kRtvarHiscoreName, "");
...
-(void) touchesMoved:(NSSet*)touches withEvent:(UIEvent*)event {
	uilepra::touch::TouchHandler::HandleTouches(touches, _canvas, Downwash::Downwash::GetApp()->drag_manager_);
}
