	...
	def apply_ini_config(self, config, group):
		...
		for section in config.sections():
			if section.startswith("engine:"):
				...
				pushengines = ["cam_flat_push", "cam_3d_push", "hover", "yaw_brake"]
				...
				tagOk = tagtype in ["eye", "brake_light", "reverse_light", "engine_sound", "exhaust", "stunt_trigger_data", "race_trigger_data", "upright_stabilizer", "forward_stabilizer", "context_path", "see_through", "anything"]
				allApplied &= tagOk
				if not tagOk:
