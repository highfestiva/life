...
class Node:
        ...
        # Jonte's addition.
        def getparents(self):
                """Return all parents - may be more than one - in a list.
                """
...
#                        opts[name] = map(lambda x: stripQuotes(x), opts[name])
        ...
        def onParent(self, objects, parent, opts):
                ...
                parent = stripQuotes(parent)
                node = self.findNode(objects[0], create=False)
                parentNode = self.findNode(parent, create=False)
...
#                        opts[name] = map(lambda x: stripQuotes(x), opts[name])
                ...
                if attr[0]!=".":
                        print("mayaascii: Warning: DefaultMAReader.onSetAttr(): The attribute refers to a different object than the current object. This is not yet supported.", file=sys.stderr)
...
#                        opts[name] = map(lambda x: stripQuotes(x), opts[name])
                ...
                if sn!=None:
                        if dn==None:
                                print('WARNING: %s, %d: connectAttr "%s" "%s"'%(self.filename, self.linenr, srcattr, dstattr), file=sys.stderr)
                                print(' Node "%s" not found. The connection is ignored.'%dnode, file=sys.stderr)
                        else:
                ...
                for name in names:
                        ...
                        if name=="":
                                ...
                                if node==None:
                                        ...
                                        if node==None:
                                                if create:
                                                        #print("Creating <unknown> node:", name)
