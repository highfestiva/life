...
fasttap = lambda: taps()[0].isrelease and timein(0.4, timer='tap')
holdtap = lambda: taps()[0].movement2()<3e-5 and timeout(0.6, timer='tap')
...
while loop():
    ...
    if click() and (is_touch_device() or timeout(0.2, timer='build', first_hit=True)):
