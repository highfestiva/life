...
- (void)commonSetup {
	...
	self.tokens = [self solverTokens_];
}


- (NSArray *)solverTokens_
{
	NSArray *solverTokens_ = @[
	   [CYRToken tokenWithName:@"operator"
			expression:@"[%/\\*,\\;:=<>\\+\\-\\^!&|~]"
			attributes:@{
				 NSForegroundColorAttributeName : kRgb(245, 0, 110)
				 }],
	   [CYRToken tokenWithName:@"round_brackets"
			expression:@"[\\(\\)]"
			attributes:@{
				 NSForegroundColorAttributeName : kRgb(161, 75, 0)
				 }],
	   [CYRToken tokenWithName:@"other_brackets"
			expression:@"[\\[\\]{}]"
			attributes:@{
				 NSForegroundColorAttributeName : kRgb(105, 0, 0),
				 NSFontAttributeName : self.boldFont
				 }],
	   [CYRToken tokenWithName:@"hex"
			expression:@"\\b0x[0-9a-fA-F]+\\b"
			attributes:@{
				 NSForegroundColorAttributeName : kRgb(0, 0, 255)
				 }],
	   ...
	   [CYRToken tokenWithName:@"reserved_words"
			...
			attributes:@{
				 NSForegroundColorAttributeName : kRgb(104, 0, 111),
				 NSFontAttributeName : self.boldFont
				 }],
	   [CYRToken tokenWithName:@"double_quoted_string"
			expression:@"r*\".*?(\"|$)"
			attributes:@{
				 NSForegroundColorAttributeName : kRgb(24, 110, 109)
				 }],
	   ...
	   [CYRToken tokenWithName:@"comment"
			...
			attributes:@{
				 NSForegroundColorAttributeName : kRgb(31, 131, 0),
				 NSFontAttributeName : self.italicFont
				 }],
	];
	return solverTokens_;
}
...
+(CGSize) slowFitTextSize:(NSString*)text {
	PythonTextView* view = [[PythonTextView alloc] initWithFrame:CGRectMake(0, 0, 300, 300)];
	view.text = text;
	return [view fitTextSize];
}
