...
pusher = ship.create_engine(push_rel_engine, strength=20, friction=0.9)	# For forward acceleration.
pusher.addsound(sound_engine_hizz, intensity=0.2, volume=0.2)
steer = ship.create_engine(push_turn_rel_engine, friction=2)	# Handles pitch and roll.

cam(angle=(-0.2,0,0), distance=20, target=ship, target_relative_angle=True)
fog(230,350)
...
def create_room(pos):
	...
	wpos,cubepos,nozfight = pos*grid_scale,vec3(0,grid_scale*0.5,grid_scale*0.2),vec3(0.1,0.1,0.1)
		...
		for y in [front+i for i in room_prio]:
			for x in [left+i for i in room_prio]:
				...
				if key in rooms:
					...
					rooms[key] = create_room(vec3(x,y,z))
