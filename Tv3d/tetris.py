#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Tetris prototyping. Move left/right/down by tapping left/right/below tetromino. Rotate by tapping above.
...
tetromino_index,tetromino = None,None
fixed_blocks = [[None]*gridsize.x for _ in range(gridsize.y)]

while loop():
	if not tetromino:
		tetromino_index = random.choice(range(len(tetrominos)))
		tetromino = create_ascii_object(tetrominos[tetromino_index], pos=(0,0,gridsize.y/2), col=colors[tetromino_index], center=(1.5,0,0))
		orientation = quat()
		...
		for y,row in enumerate(piece):
			for x,block in enumerate(row):
				if block == 'X':
					...
					fixed_blocks[p.y+gridsize.y//2][p.x+gridsize.x//2] = create_ascii_object('X', pos=p, col=colors[tetromino_index])

		# Consume completed rows.
		ri = 0
		while ri < len(fixed_blocks):
			row = fixed_blocks[ri]
			if len([1 for block in row if block]) == gridsize.x:
