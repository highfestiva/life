...
car.weight(500)
color('#222')
wheel = lambda x,y,z:create_sphere_object(pos=(x,y,z),radius=1.1)
rr,rl,fr,fl = wheel(-4,-2.5,-0.7), wheel(-4,+2.5,-0.7), wheel(3,-2.5,-0.7), wheel(3,+2.5,-0.7)
car.joint(hinge2_joint, fl, (0,+1,0))
turn = [car.create_engine(turn_engine)]
car.joint(hinge2_joint, fr, (0,-1,0))
turn += [car.create_engine(turn_engine)]
car.joint(hinge_joint, rl, (0,+1,0))
roll = [car.create_engine(roll_engine)]
car.joint(hinge_joint, rr, (0,-1,0))
roll += [car.create_engine(roll_engine)]

# Setup ground, gravity, camera angle and controls.
create_static_cube(pos=(0,0,-252), size=500)
gravity((0,0,-9))
cam(angle=(-0.3,0,0), dist=20, target=car)
