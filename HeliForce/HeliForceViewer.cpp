...
HeliForceViewer::HeliForceViewer(Life::GameClientMasterTicker* pMaster, const Cure::TimeManager* pTime,
...
HeliForceViewer::~HeliForceViewer()
{
	CloseJoinServerView();
}
...
void HeliForceViewer::LoadSettings()
{
...
void HeliForceViewer::SaveSettings()
{
}

bool HeliForceViewer::Open()
{
	return Life::GameClientSlaveManager::Open();
}

void HeliForceViewer::TickUiInput()
{
}

void HeliForceViewer::TickUiUpdate()
{
...
void HeliForceViewer::CreateLoginView()
{
	QuaternionF lFlip;
...
bool HeliForceViewer::InitializeUniverse()
{
	...
	Cure::ContextObject* lVehicle = new UiCure::Machine(GetResourceManager(), _T("heli_force_01"), mUiManager);
...
void HeliForceViewer::OnLoadCompleted(Cure::ContextObject* pObject, bool pOk)
{
	if (pOk)
	{
	}
...
void HeliForceViewer::OnCancelJoinServer()
{
	CloseJoinServerView();
}

void HeliForceViewer::OnRequestJoinServer(const str& pServerAddress)
{
...
bool HeliForceViewer::UpdateServerList(Life::ServerInfoList& pServerList) const
{
...
bool HeliForceViewer::IsMasterServerConnectError() const
{
	if (GetMaster()->GetMasterServerConnection())	// TRICKY: uses master ticker's connection, as this is the one that downloads server lists!
	{
...
void HeliForceViewer::CloseJoinServerView()
{
	if (mServerListView)
	{
...
RoadSignButton* HeliForceViewer::CreateButton(float x, float y, float z, const str& pName, const str& pClass, const str& pTexture, RoadSignButton::Shape pShape)
{
	...
	lButton->GetButton().SetOnClick(HeliForceViewer, OnButtonClick);
	mRoadSignMap.insert(RoadSignMap::value_type(lButton->GetInstanceId(), lButton));
	lButton->StartLoading();
	return (lButton);
}

void HeliForceViewer::OnButtonClick(UiTbc::Button* pButton)
{
...
LOG_CLASS_DEFINE(GAME, HeliForceViewer);
