...
#  endif
  ...
  _FreeBlockHeader* __block123 = __STATIC_CAST(_FreeBlockHeader*, _S_free_mem_blocks.pop());
  if (__block123 != 0) {
    ...
    char*  __buf_start  = __REINTERPRET_CAST(char*, __block123);
    __add_atomic_t __bytes_left = __block123->_M_end - __buf_start;
    ...
    if (__bytes_left != 0) {
      ...
      if ((__result != 0) && (__bytes_left >= (__add_atomic_t)sizeof(_FreeBlockHeader))) {
        ...
        __newblock->_M_end  = __block123->_M_end;
