...
static dJointID joint[4];	// joint[0]+joint[1] are the front wheels
...
{
    // motor
    dJointSetHinge2Param (joint[x],dParamVel2,-speed);
    dJointSetHinge2Param (joint[x],dParamFMax2,0.1);

    // steering
    dReal v = steer - dJointGetHinge2Angle1 (joint[x]);
    //if (v > 0.1) v = 0.1;
    //if (v < -0.1) v = -0.1;
    v *= 1.0;
    dJointSetHinge2Param (joint[x],dParamVel,v);
    dJointSetHinge2Param (joint[x],dParamFMax,0.2);
    dJointSetHinge2Param (joint[x],dParamLoStop,-0.75);
    dJointSetHinge2Param (joint[x],dParamHiStop,0.75);
    dJointSetHinge2Param (joint[x],dParamFudgeFactor,0.1);
}
