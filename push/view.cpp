...
View::View(const wstr& title, uitbc::Layout* layout):
	uitbc::Window(uitbc::Window::kBorderLinearshading, 2, DARK_GRAY, layout) {
	Init();
	...
	uitbc::Button* close_button = new uitbc::Button(uitbc::BorderComponent::kZigzag, 1, LIGHT_RED, L"");
...
uitbc::RectComponent* View::AddRow(const Color& color, int column_count) {
	uitbc::RectComponent* row_layer = new uitbc::RectComponent(color, new uitbc::GridLayout(1, column_count));
...
uitbc::RectComponent* View::AddCentering(int layer, uitbc::Component* parent) {
	uitbc::RectComponent* center_layer = new uitbc::RectComponent(new uitbc::CenterLayout);
	parent->AddChild(center_layer, 0, 0, layer);
	return center_layer;
}

uitbc::Label* View::AddLabel(const wstr& text, const Color& color, int preferred_width, uitbc::Component* parent, int layer) {
...
uitbc::TextField* View::AddTextField(const wstr& default_text, const str& name) {
...
uitbc::Button* View::AddButton(const wstr& text, void* extra_data, uitbc::Component* parent) {
