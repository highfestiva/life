...
+(void) updateContent
{
	...
	CURE_RTVAR_SET(TireFire::App::GetApp()->mVariableScope, RTVAR_CONTENT_LEVELS, (hasLevels == 1));

	NSInteger hasVehicles = [defaults integerForKey:@ CONTENT_VEHICLES];
	CURE_RTVAR_SET(TireFire::App::GetApp()->mVariableScope, RTVAR_CONTENT_VEHICLES, (hasVehicles == 1));

	NSString* objcHiscoreName = [defaults stringForKey:HISCORE_NAME_KEY];
	const str hiscoreName = MacLog::Decode(objcHiscoreName);
	CURE_RTVAR_SET(TireFire::App::GetApp()->mVariableScope, RTVAR_HISCORE_NAME, hiscoreName);
}

+(void) storeHiscoreName
{
	str lLastHiscoreName;
	CURE_RTVAR_GET(lLastHiscoreName, =, TireFire::App::GetApp()->mVariableScope, RTVAR_HISCORE_NAME, _T(""));
...
-(TireFire::FingerMovement&) getFingerMovement:(const CGPoint&)pLocation previous:(const CGPoint&)pPrevious
{
	TireFire::FingerMoveList::iterator i = TireFire::gFingerMoveList.begin();
	for (; i != TireFire::gFingerMoveList.end(); ++i)
	{
	...
	TireFire::gFingerMoveList.push_back(TireFire::FingerMovement(pLocation.x, pLocation.y));
	return TireFire::gFingerMoveList.back();
}

-(void) dropFingerMovements
{
	TireFire::FingerMoveList::iterator i = TireFire::gFingerMoveList.begin();
	for (; i != TireFire::gFingerMoveList.end();)
	{
		if (!i->mIsPress)
		{
			TireFire::gFingerMoveList.erase(i++);
			//return;
		}
		else
		{
...
-(void) touchesMoved:(NSSet*)touches withEvent:(UIEvent*)event
{
	...
	while ((lTouch = (UITouch*)[e nextObject]))
	{
		...
		TireFire::FingerMovement& lMove = [self getFingerMovement:lTapPosition previous:lPrevTapPosition];
		lMove.mIsPress = lIsPressed;
		/*TireFire::App::OnTap(lMove);
		if (!lIsPressed)
		{
			[self dropFingerMovement:lTapPosition previous:lPrevTapPosition];
		}*/

		//TireFire::App::OnMouseTap(lTapPosition.x, lTapPosition.y, lIsPressed);
	}
}
